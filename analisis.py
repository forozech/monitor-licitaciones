        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="text-end">
                <span class="badge bg-white text-secondary border shadow-sm py-2 px-3">
                    <i class="bi bi-clock me-1"></i> Actualizado: <span id="lastUpdate">__FECHA__</span>
                </span>
            </div>
        </div>
...
# --- MOTOR DE EXTRACCIÓN (RSS CORREGIDOS) ---
class MonitorEngine:
    def __init__(self):
        # NOTA: He reconstruido el enlace de Servicios-Formalizado basándome en los otros
        # Todos filtran por fecha >= 01/01/2025 y Gipuzkoa (ES212)
        self.sources = [
            # 1. OBRAS (p01=1)
            {
                "tipo": "OBRA", "estado": "ADJUDICADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=1&p02=5&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            },
            {
                "tipo": "OBRA", "estado": "FORMALIZADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=1&p02=8&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            },
            {
                "tipo": "OBRA", "estado": "CERRADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=1&p02=14&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            },
            # 2. SERVICIOS (p01=2)
            {
                "tipo": "SERV", "estado": "ADJUDICADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=2&p02=5&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            },
            {   # ENLACE REPARADO: Servicios (p01=2) + Formalizado (p02=8) + Filtros
                "tipo": "SERV", "estado": "FORMALIZADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=2&p02=8&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            },
            {
                "tipo": "SERV", "estado": "CERRADO",
                "url": "https://www.contratacion.euskadi.eus/ac70cPublicidadWar/suscribirAnuncio/suscripcionRss?p01=2&p02=14&p11=01/01/2025&p26=ES212&p45=1&idioma=es&R01HNoPortal=true"
            }
        ]
    ...
    def get_tab_value(self, soup, tab_id, label):
        ...
        # Búsqueda robusta en filas Bootstrap
        for row in tab.find_all('div', class_=re.compile(r'row|fila')):
            text_row = row.get_text(" ", strip=True).lower()
            if label.lower() in text_row:
                cols = row.find_all('div', class_=re.compile(r'col-'))
                if len(cols) > 1: return cols[1].get_text(strip=True)
                
        # Búsqueda fallback en listas de definición
        dl = tab.find_all('dl')
        for list_item in dl:
            dts = list_item.find_all('dt')
            dds = list_item.find_all('dd')
            for i, dt in enumerate(dts):
                if label.lower() in dt.get_text(strip=True).lower():
                    if i < len(dds): return dds[i].get_text(strip=True)
        return None
    ...
    def process_url(self, url, tipo, estado):
        ...
        try:
            ...
            # Intentar sacar presupuesto de varias etiquetas posibles
            base_str = self.get_tab_value(soup, 'tabs-4', 'Presupuesto del contrato sin IVA')
            if not base_str: base_str = self.get_tab_value(soup, 'tabs-4', 'Presupuesto base de licitación')
            
            # Intentar sacar adjudicación
            adj_str = self.get_tab_value(soup, 'tabs-9', 'Precio sin IVA')
            if not adj_str: adj_str = self.get_tab_value(soup, 'tabs-9', 'Importe de adjudicación')
            ...
            if tab8:
                for row in tab8.find_all('div', class_='row'):
                    ...
                    # Filtro simple para detectar filas con nombres de empresa
                    if "Razón Social" not in txt and len(txt) > 3:
                         cols = row.find_all('div', class_=re.compile(r'col-.*-8'))
                         if cols: rivales.append(cols[0].get_text(strip=True))
    ...
    def run(self):
        print("🚀 INICIANDO MONITOR DE ADJUDICACIONES...")
        results = []
        
        for s in self.sources:
            print(f"📡 Leyendo RSS: {s['tipo']} - {s['estado']}")
            try:
                ...
                # Límite de seguridad: procesar últimos 15 items por feed
